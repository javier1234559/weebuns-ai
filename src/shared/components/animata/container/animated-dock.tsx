import { cn } from "@/shared/lib/utils"; // Import utility for conditional class names
import {
  ...
  return (
    <div className={cn("relative block md:hidden", className)}>
            ...
            {items.map((item, index) => (
              ...
              >
                <Link
                  href={item.href}
                  key={item.title}
                  className="flex size-10 items-center justify-center rounded-full bg-white/20 text-black shadow-md backdrop-blur-md dark:bg-black/20 dark:text-white"
                >
                  <div className="size-4">{item.icon}</div> {/* Render the icon */}
                </Link>
              </motion.div>
            ))}
          </motion.div>
      ...
      >
        ...
        {isOpen ? (
          <X className="size-5" /> // Show close icon when open
        ) : (
          <Menu className="size-5" /> // Show menu icon when closed
        )}
      </button>
    </div>
  );
